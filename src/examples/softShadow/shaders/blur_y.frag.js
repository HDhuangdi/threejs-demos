export default `
uniform sampler2D u_blur_x_texture;
...
vec4 box_blur_y(sampler2D tex, vec2 uv, float texture_height, float radius) {
    vec4 color = texture2D(tex, uv);
	vec3 sum_c = color.rgb;
   	vec2 offset = vec2(0.0, 1.0 / texture_height);
...
void main() {
    vec4 blur_color = gaussian_blur(vec2(u_texture_width, u_texture_height), u_blur_x_texture, v_uv, vec2(0.0, 1.0));
    vec4 color = texture2D(u_blur_x_texture, v_uv);
